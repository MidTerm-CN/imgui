...
static void ImGui_ImplWin32_UpdateGamepads(ImGuiIO& io)
{
    ...
    if (bd->PrevGamepadState.dwPacketNumber == xinput_state.dwPacketNumber)
        return;

#define IM_SATURATE(V)                      (V < 0.0f ? 0.0f : V > 1.0f ? 1.0f : V)
#define MAP_ANALOG(KEY_NO, VALUE, V0, V1)   { float vn = (float)(VALUE - V0) / (float)(V1 - V0); io.AddKeyAnalogEvent(KEY_NO, vn > 0.10f, IM_SATURATE(vn)); }
    // Each packet contains only one state change
    if (bd->PrevGamepadState.Gamepad.wButtons != gamepad.wButtons)
    {
        for (int n = 0; n < 16; n++)
        {
            if ((gamepad.wButtons & (1 << n)) == (bd->PrevGamepadState.Gamepad.wButtons & (1 << n)))
                continue;
    ...
    else if (bd->PrevGamepadState.Gamepad.bLeftTrigger != gamepad.bLeftTrigger)
    {
        MAP_ANALOG(ImGuiKey_GamepadL2, gamepad.bLeftTrigger, XINPUT_GAMEPAD_TRIGGER_THRESHOLD, 255);
    }
    else if (bd->PrevGamepadState.Gamepad.bRightTrigger != gamepad.bRightTrigger)
    {
        MAP_ANALOG(ImGuiKey_GamepadR2, gamepad.bRightTrigger, XINPUT_GAMEPAD_TRIGGER_THRESHOLD, 255);
    }
    else if (bd->PrevGamepadState.Gamepad.sThumbLX != gamepad.sThumbLX)
    {
        MAP_ANALOG(ImGuiKey_GamepadLStickLeft, gamepad.sThumbLX, -XINPUT_GAMEPAD_LEFT_THUMB_DEADZONE, -32768);
        MAP_ANALOG(ImGuiKey_GamepadLStickRight, gamepad.sThumbLX, +XINPUT_GAMEPAD_LEFT_THUMB_DEADZONE, +32767);
    }
    else if (bd->PrevGamepadState.Gamepad.sThumbLY != gamepad.sThumbLY)
    {
        MAP_ANALOG(ImGuiKey_GamepadLStickUp, gamepad.sThumbLY, +XINPUT_GAMEPAD_LEFT_THUMB_DEADZONE, +32767);
        MAP_ANALOG(ImGuiKey_GamepadLStickDown, gamepad.sThumbLY, -XINPUT_GAMEPAD_LEFT_THUMB_DEADZONE, -32768);
    }
    else if (bd->PrevGamepadState.Gamepad.sThumbRX != gamepad.sThumbRX)
    {
        MAP_ANALOG(ImGuiKey_GamepadRStickLeft, gamepad.sThumbRX, -XINPUT_GAMEPAD_LEFT_THUMB_DEADZONE, -32768);
        MAP_ANALOG(ImGuiKey_GamepadRStickRight, gamepad.sThumbRX, +XINPUT_GAMEPAD_LEFT_THUMB_DEADZONE, +32767);
    }
    else if (bd->PrevGamepadState.Gamepad.sThumbRY != gamepad.sThumbRY)
    {
...
#undef MAP_BUTTON
#undef MAP_ANALOG
    bd->PrevGamepadState = xinput_state;
